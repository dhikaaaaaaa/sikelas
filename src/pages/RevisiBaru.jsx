...
export default function RevisiBaru() {
  ...
  const [form, setForm] = useState({ classId: '', sessionDate: '', currentStatus: 'alpa', reason: '' })
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5 rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700">Tanggal Sesi yang Direvisi</label>
          <input
            required
            type="date"
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700">Status Saat Ini di Sistem</label>
          <select
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700">Alasan Revisi</label>
          <textarea
            required
            rows={4}
            value={form.reason}
            onChange={(e) => update('reason', e.target.value)}
            placeholder="Jelaskan alasan mengapa status kehadiran ini salah dan perlu diperbaiki (misal: gagal scan QR presensi)..."
            className="mt-1.5 w-full rounded-lg border border-ink-200 px-3 py-2 text-sm focus:border-amber-400 focus:ring-1 focus:ring-amber-400 outline-hidden transition-all"
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-ink-700">Bukti Kehadiran (Pendukung)</label>
          <div
            ...
            <div className="text-center space-y-2 w-full flex flex-col items-center">
              {previewUrl ? (
                ...
                <>
                  ...
                  <p className="text-xs text-ink-400">Tangkapan layar presensi, foto di kelas (PDF/Gambar)</p>
                </>
              )}
            </div>
          </div>
